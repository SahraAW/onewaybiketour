export function MapGraphic({ detail = false }: { detail?: boolean }) {
  return <svg viewBox="0 0 500 600" className={`h-full w-full ${detail ? "min-h-[440px]" : ""}`} role="img" aria-label="Abstract bike tour route map">
    ...
    {detail && <><path d="M30 550 C150 470 90 372 235 350 S383 285 460 320M40 170 C140 200 136 108 230 125 S345 220 470 190" fill="none" stroke="currentColor" strokeWidth="1" opacity=".18" strokeDasharray="4 8" /><text x="91" y="536" fill="currentColor" fontSize="10" letterSpacing="2">START</text><text x="365" y="61" fill="currentColor" fontSize="10" letterSpacing="2">END</text></>}
  </svg>;
}
