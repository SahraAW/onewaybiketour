...
function AuthFrame({ eyebrow, title, footer, children }: Readonly<{ eyebrow: string; title: string; footer: React.ReactNode; children: React.ReactNode }>) { return <main className="grain flex min-h-[calc(100svh-104px)] items-start justify-center overflow-y-auto bg-[#dedad6] px-5 py-8 sm:items-center sm:py-10"><div className="w-full max-w-md"><div className="rounded-[2rem] bg-paper p-7 shadow-sm md:p-12"><p className="eyebrow mb-5 text-orange">{eyebrow}</p><h1 className="display text-6xl font-bold">{title}<span className="text-orange">.</span></h1>{children}<p className="mt-8 text-xs text-ink/60">{footer}</p></div><NavigationSteps step={2} /><Link href="/" className="mt-7 block text-center text-xs font-bold">← Back home</Link><p className="mt-3 text-center text-[10px] text-ink/40">Demo mode: no server account is created.</p></div></main>; }
