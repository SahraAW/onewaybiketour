...
export function RegisterForm() {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); setError(""); if (password.length < 6) return setError("Password must be at least 6 characters."); if (password !== confirm) return setError("Passwords do not match."); setLoading(true); window.setTimeout(() => { saveSession(name, email); router.push("/onboarding"); }, 450); };
...
function AuthFrame({ eyebrow, title, footer, children }: { eyebrow: string; title: string; footer: React.ReactNode; children: React.ReactNode }) { return <main className="grain flex min-h-screen items-center justify-center bg-[#dedad6] px-5 py-10"><div className="w-full max-w-md"><Logo /><div className="mt-20 rounded-[2rem] bg-paper p-7 shadow-sm md:p-12"><p className="eyebrow mb-5 text-orange">{eyebrow}</p><h1 className="display text-6xl font-bold">{title}<span className="text-orange">.</span></h1>{children}<p className="mt-8 text-xs text-ink/60">{footer}</p></div><Link href="/" className="mt-7 block text-center text-xs font-bold">← Back home</Link><p className="mt-3 text-center text-[10px] text-ink/40">Demo mode: no server account is created.</p></div></main>; }
