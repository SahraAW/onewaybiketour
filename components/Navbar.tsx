...
export function Navbar() {
  ...
  return <header className="mx-auto flex w-full max-w-7xl flex-wrap items-center justify-between gap-3 px-5 py-6 md:px-10">
    <Logo />
    <nav className="ml-auto hidden items-center gap-7 text-[11px] font-bold lg:flex">
    ...
    <div className="ml-auto flex items-center gap-2 lg:ml-4">
      <Link href="/onboarding" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-[20px] bg-orange px-4 py-2.5 text-[10px] font-semibold text-white transition-colors hover:bg-[#d94f00]">Find your match <span aria-hidden="true">↗</span></Link>
      <button type="button" onClick={() => setOpen(true)} aria-expanded={open} aria-controls="mobile-menu" className="min-h-11 rounded-full border border-black px-4 text-[11px] font-bold lg:hidden">Menu</button>
    </div>
